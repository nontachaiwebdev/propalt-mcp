...
export const audienceSellingPropertyInputSchema = {
  ...
  sector_code_list: z.array(z.string()).max(4).optional().describe("List of UK postcode sectors (e.g. ['NW3 1', 'NW3 2']), max 4."),
  ...
  progress: z.enum(["reduction", "for_sale", "withdrawn", "sold_sstc"]).optional().describe("Listing progress filter. Default 'for_sale'."),
  ...
  taxband_rating: z.string().optional().describe("Council tax band rating. Valid input N or N-M, A<=N<M<=I."),
  ...
  archive_date: z.string().optional().describe("Valid formats: DDMMYYYY-DDMMYYYY or min_week:N|max_week:M or min_month:N|max_month:M."),
  sold_date: z.string().optional().describe("Valid formats: DDMMYYYY-DDMMYYYY or min_week:N|max_week:M or min_month:N|max_month:M."),
  listing_price: z.string().optional().describe("Listing price. Valid input N or N-M, sale:10000<=N<M."),
...
export const audienceSellingPropertyDescription =
...
export function makeAudienceSellingPropertyHandler(api: ApiClient) {
  return async (input: Record<string, unknown>) => {
    const data = await api.request("POST", "/audience/selling-property", { body: input });
    return {
